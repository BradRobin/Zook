...
class StreamProcessor:
    ...
    def __init__(
        ...
        logger.info(f"StreamProcessor created for session {session.display_session_id()}")
    
    async def start(self):
        """Start the frame processing loop."""
        if self.is_running:
            logger.warning(
                f"StreamProcessor already running for session {self.session.display_session_id()}"
            )
            return
        
        self.is_running = True
        self._process_task = asyncio.create_task(self._process_loop())
        logger.info(f"StreamProcessor started for session {self.session.display_session_id()}")
    ...
    async def stop(self, reason: str = "disconnect"):
        ...
        logger.info(
            f"StreamProcessor stopped for session {self.session.display_session_id()} "
    ...
    async def _process_loop(self):
        ...
        """
        logger.info(f"Processing loop started for session {self.session.display_session_id()}")
        ...
        while self.is_running and self.session.is_active:
            try:
                ...
                logger.info(
                    f"Processing loop cancelled for session {self.session.display_session_id()}"
                )
                break
        ...
        logger.info(f"Processing loop ended for session {self.session.display_session_id()}")
    ...
    async def _process_frame(self, frame_data: dict):
        ...
        try:
            ...
            await self._send_result({
                ...
                'session_id': self.session.display_session_id(),
            ...
            if frame_data['frame_number'] % 100 == 0:
                logger.info(
                    f"Session {self.session.display_session_id()}: "
